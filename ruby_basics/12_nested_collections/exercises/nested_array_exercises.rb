...
def count_empty_seats(chart)
  ...
  number_of_empty_seats  = chart.reduce(0) do |total_nil,row| 
    row_total_nils= row.reduce(0) do |acc,col| 
      acc+=1 if col.nil?  
      acc
    end
    total_nil += row_total_nils
    total_nil
  end 
...
def find_favorite(array_of_hash_objects)
  ...
  return array_of_hash_objects.find  do |obj|
